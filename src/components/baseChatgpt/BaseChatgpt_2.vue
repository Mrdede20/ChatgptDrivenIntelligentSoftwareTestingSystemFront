<template>
  ...
  >
    ...
    >
      ...
      <div class="el-upload__text">将正确程序拖到此处，或<em>点击上传</em></div>
      <div class="el-upload__tip" slot="tip">只能上传 Python 文件，且不超过500MB，上传好的文件如下：</div>
    </el-upload>
    <div class="button-container">
      <el-button type="primary" round @click="toBaseChatgpt_3">BaseChatGpt</el-button>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      pathParam: 'correct',  // 可以根据实际情况动态更新这个值
      activeIndex: 2,
      is_processing: false,
      files: [],
    };
  ...
  methods: {
    toBaseChatgpt_3(){
     	this.$router.push({ path: "/BaseChatgpt_3" });
    },
