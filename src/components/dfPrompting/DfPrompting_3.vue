<template>
  ...
  >
    ...
    >
      ...
      <div class="el-upload__text">将程序意图拖到此处，或<em>点击上传</em></div>
      <div class="el-upload__tip" slot="tip">只能上传 Txt 文件，且不超过500MB，上传好的文件如下：</div>
    </el-upload>
    <div class="button-container">
      <el-button type="primary" round @click="toDfPrompting_4">DfPrompting</el-button>
    </div>
  </div>
</template>
...
export default {
  data() {
    return {
      pathParam: 'intention',  // 可以根据实际情况动态更新这个值
      activeIndex: 3,
      is_processing: false,
      files: [],
    };
  ...
  methods: {
    toDfPrompting_4(){
     	this.$router.push({ path: "/DfPrompting_4" });
    },
    ...
    beforeFileUpload(file) {
      const isPython = file.name.split('.').pop().toLowerCase() === 'txt';
      if (!isPython) {
